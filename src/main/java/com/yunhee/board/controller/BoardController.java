...
public class BoardController {
    ...
    public String findById(@RequestParam("id") Long id, Model model) { // @RequestParam 뜻 : id라는 파라미터에 담겨온 값을 Long 타입의 id 변수에 담겠다 / 마찬가지로 DB에서 데이터를 가져오는 것이기 때문에 Model 필요
    ...
    public String update(@ModelAttribute BoardDTO boardDTO, Model model) {
        boardService.update(boardDTO); // 글 수정
        BoardDTO dto = boardService.findById(boardDTO.getId()); // 수정된 글의 상세를 가져오기 위해 findById 메서드 사용(수정된 boardDTO 객체의 id값에 해당하는 글을 가져오기 때문에 boardDTO.getId()를 매개변수로 넣어줌
